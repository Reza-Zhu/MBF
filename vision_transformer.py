...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.last_block = nn.Sequential(*[
            block_fn(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, init_values=init_values,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[11], norm_layer=norm_layer, act_layer=act_layer)
        ])
    ...
    def forward_features(self, x, t):
        x, patch_feature = self.patch_embed(x)
        ...
        return x, patch_feature, last_feature
    ...
    def forward(self, x, t):
        x, patch_feature, last_feature = self.forward_features(x, t)
        # print(x.shape)
        vit_feature = x
        x = self.forward_head(x)
        return x, patch_feature, vit_feature, last_feature
