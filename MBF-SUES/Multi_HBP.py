...
class Hybird_ViT(nn.Module):
    def __init__(self, classes, drop_rate, block, share_weight=True):
        ...
        self.p = nn.Parameter(torch.ones(1024)*3, requires_grad=True)
        self.gem = GeM(dim=1024, p=self.p)
    ...
    def fusion_features(self, x, t, model):
        ...
        # after lpn HBP softmax
        # lpn = self.hbp(result[0], result[1])
        # y4 = self.classifier_hbp(lpn)
...
if __name__ == '__main__':
    # create_model()
    model = Hybird_ViT(classes=120, drop_rate=0.3, block=2).cuda()
    # print(model)
